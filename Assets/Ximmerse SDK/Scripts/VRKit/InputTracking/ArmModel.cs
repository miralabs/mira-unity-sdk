...
*/
  ...
  public void OnControllerUpdate() {
    ...
    //
    if (IsControllerRecentered()) {
      ResetState();
    }

    UpdateHandedness();
    UpdateTorsoDirection();
    if (Controller.connectionState == DeviceConnectionState.Connected) {
  ...
  private void UpdateHandedness() {
    ...
    //Settings.UserPrefsHandedness handedness = Settings.Handedness;

    // Determine handedness multiplier.
    handedMultiplier.Set(0, 1, 1);
    if (handedness == ControllerType.RightController) {
      handedMultiplier.x = 1.0f;
    } else if (handedness == ControllerType.LeftController) {
      handedMultiplier.x = -1.0f;
    }
...
*/
  ...
  private void UpdateTorsoDirection() {
    ...
    if (followGaze == GazeBehavior.Always || firstUpdate) {
      ...
      float angularVelocity = Controller.GetGyroscope().magnitude;
    ...
    Quaternion gazeRotation = Quaternion.LookRotation(torsoDirection, Vector3.up);//Quaternion.FromToRotation(Vector3.forward, torsoDirection);
  ...
  private void UpdateFromController() {
    ...
    Vector3 accel = Controller.GetRotation() * Controller.GetAccelerometer();
...
/*
    ControllerVisual.AlphaValue = Mathf.Max(alphaValue, ControllerVisual.AlphaValue);
*/
  }
